<script setup>
  ...
  import { validateName, errPublicherEmp, errPublicher } from '@/assets/javascript/validation'
  ...
  <div class="page-wrap">
    <section class="form">
      <div class="title-wrap">
        <h1 v-if="method == 'post'">Add publicher</h1>
        <h1 v-else>Update publicher</h1>
      </div>
...
export default {
  ...
  methods: {
    validate() {
      this.nameError = this.name.length <= 0 ? errPublicherEmp() : 
      (validateName(this.name) ? '' : errPublicher(this.name)) 
    },
    async submit() {
      this.validate();
      if(this.nameError == ''){
...
@import "../../assets/styles/forms/publicherForm.scss";
</style>